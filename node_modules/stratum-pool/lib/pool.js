...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupRecipients() {
        ...
        options.ac_founders = options.ac_founders || {};
        ...
        for (var r in options.ac_founders) {
            var percent = options.ac_founders[r];
            var rObj = {
                founders: percent,
                address: r
            };
    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            if (options.coin.disablecb) {
              if (typeof(rpcResults.getinfo.pubkey) === 'undefined') {
                  if ( typeof(rpcResults.validateaddress.pubkey) === 'undefined' ) {
                    emitErrorLog('pubkey is not set, and pool address is not imported. You must either import the private key or set the daemons -pubkey when using disable coinbase mode.');
                    return;
                  } else {
                    var pubkey = rpcResults.validateaddress.pubkey;
                  }
              } else {
                  var pubkey = rpcResults.getinfo.pubkey;
              }
              _this.daemon.cmd('setpubkey', [pubkey], function (results) {
                    if (results.error) {
                        emitErrorLog('rpc error with daemon instance ' +
                            results.instance.index + ' when submitting block with ' + rpcCommand + ' ' +
                            JSON.stringify(results.error)
                        );
                        return;
                    } else {
                      if ( results[0].response.address !== options.address) {
                          emitErrorLog('The pool address ' + options.address + ' does not match the pubkeys address ' + results[0].response.address);
                          return;
                      }
                    }
                });
    ...
    function GetBlockTemplate(callback) {
        function getNextBlockHeight() {
            _this.daemon.cmd(
                'getblockcount',
                [],
                result => result.error ? callback(result.error) : getBlockSubsidyandTemplate(result[0].response)
            )
        }

        function getBlockSubsidyandTemplate(height) {
            _this.daemon.cmd(
                'getblocksubsidy',
                [(height+1)],
        ...
        function getBlockTemplate(subsidy) {
            ...
            if (options.coin.disablecb) {
                var jsontosend = [{"mode":"disablecb"}];
            } else {
                var jsontosend = [{"capabilities": ["coinbasetxn", "workid", "coinbase/append"]}];
            }
            _this.daemon.cmd('getblocktemplate',jsontosend,
                function (result) {
                    if (result.error) {
