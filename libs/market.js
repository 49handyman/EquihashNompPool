...
var portalConfig = process.env.portalConfig;
const loggerFactory = require('./logger.js');
const logger = loggerFactory.getLogger('Market', 'system');


var poolConfigs = process.env.pools;
//console.log(process.env.pools);
var enabledPools = [];
/*
...
async.filter(enabledPools, function(coin, callback){
    ...
    results.forEach(function(coin){
        ...
        logger.info( 'Wallet stats setup with daemon ('
...
//console.log('poolOptions: '+ poolOptions+'enable pools: '+enabledPools);

/**
 * Get market prices
 **/
//console.log('1'+exchange+tickers)
exports.get = function(exchange, tickers, callback) {
 ...
 //   console.log('2'+exchange+tickers)
...
//console.log('0 '+pairName+pairParts+base+target)
 ...
 //                                   console.log('1.2: '+marketPrices+tickerData)
                                }
                                if (completedFetches === numTickers) callback(marketPrices);
...
//console.log('3 '+exchange+tickers)
getExchangeMarkets(exchange);

function getExchangeMarkets(exchange, callback) {
    exchange = "tradeogre"
    callback = callback || function(){};
 //   console.log('4 '+exchange+tickers)
    if (!exchange) { 
 //       console.log('4.1 '+exchange+tickers)
        callback('No exchange specified', null);
    }
    exchange = exchange.toLowerCase();
 //   console.log('4.2 '+exchange+tickers)
    // Return cache if available
    let cacheKey = exchange;
    let currentTimestamp = Date.now() / 1000;
  //  console.log('4.3 '+exchange+tickers+currentTimestamp)
 ...
 //       return ;
  //  }
    ...
    let price = 0.0;
    let data = {};
//    console.log('6 '+exchange+tickers)
    exchange = 'tradeogre';
    // Altex
    if (exchange == "altex") {
    ...
    // TradeOgre
        ...
        request('https://tradeogre.com/api/v1/markets', function(error, response,body) {
        //    console.log('7.1: body: '+ body+'error: '+error)
            if (error) console.log('API request to %s has failed: %s', [exchange, error]);
        //    var price = data[coin].usd
            let pairParts = null;
            if (!error && body) {
                var data = JSON.parse(body);
             //   var price = data.price
             //   console.log('7.2 :'+ data[0][0]])
              //  console.log('7.3: '+ body)
             /*   for (let i in body) {
                    for (let pairName in data[i]) {
                        
                        pairParts = pairName.split('-');
                        target = pairParts[0];
                        symbol = pairParts[1];

 //                       console.log('7.3.5: '+ symbol)
                        price = +data[i][pairName].price;
   //                     console.log('5.1: '+ price)
                        if (price === 0) continue;
