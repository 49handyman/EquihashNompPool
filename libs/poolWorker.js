  ...
  //    logger.debug("forkId %s", forkId);
      ...
      if (data.blockHash && !isValidBlock) {
            if (workerInfo.length === 2) {
                logger.info('BLOCK>REJECTED> Found block rejected by the daemon, share data: %s\u001b[37m' + shareDataJsonStr);
            }
            else {
                logger.info('BLOCK>REJECTED> Found block rejected by the daemon, share data: %s\u001b[37m' + shareDataJsonStr);
            }
        ...
        if (workerInfo.length === 2) {
            if (isValidShare) {
                if (data.shareDiff > 1000000000) {
                    logger.warn('\u001b[36mSHARE>WARN> Share was found with diff higher than 1.000.000.000!\u001b[37m');
            ...
            if (isValidShare) {
                ...
                logger.info('SHARE>ACCEPTED> job: %s req: %s res: %s by %s worker: none [%s]\u001b[37m', data.job, data.difficulty, data.shareDiff, workerStr, functions.anonymizeIP(data.ip));                
            } 
            else if (!isValidShare) {
                logger.info('SHARE>REJECTED> job: %s diff: %s by %s worker: none reason: %s [%s]\u001b[37m', data.job, data.difficulty, workerStr, data.error, functions.anonymizeIP(data.ip));
            }
        }
