...
var mmt = moment().utc().local();
var endMidnight = mmt.clone().endOf('day').local();
var startMidnight = mmt.clone().startOf('day').local();
sinceMidnight = moment("00:00:00", "hh:mm:ss").diff(moment().local());
midnightdiff=startMidnight -endMidnight;
var diffMinutes = mmt.diff(mmtMidnight, 'minutes');
var mmt = moment().utc().local();
var date = moment().utc().local();
var mmtMidnight = mmt.clone().endOf('day');
sinceMidnight = moment("00:00:00", "hh:mm:ss").diff(moment());
console.log('time: '+readableDate(Date.now()));
...
function displayCharts() {
    ...
    [{
          label: 'Actual',
          fill: false,
          data: stats.hashrate.map(x => { return {t: x[0], y: getReadableHashRate(x[1])}} ),
          borderWidth: 1,
...
//	$("#marketCap").attr("class", "text-danger");
	...
	$("#priceUSD").html('Price USD<br>$' + parseFloat(data.market_data.current_price.usd));
        ...
        $("#volume24").html('24h Volume<br>' + parseFloat(data.market_data.total_volume.usd));
        $("#ath").html('ATH<br>$' + Number(parseFloat(data.market_data.ath.usd))+ '<BR>' +
			 (data.market_data.ath_date.usd).substring(0, 10)).attr("class", "text-success");

	$("#atl").html('ATL<br>$' + Number(parseFloat(data.market_data.atl.usd))+ '<BR>' + 
		(data.market_data.atl_date.usd).substring(0, 10)).attr("class", "text-danger");

        $("#24h_high").html('24h High<BR>$' + Number(parseFloat(data.market_data.high_24h['usd'])).toFixed(2));
...
console.log('price change: '+data.market_data.price_change_24h);
        $("#shortDesc").html('All Data Provided By Coingecko: '+data.id + ': ' + short_desc || undefined);
...
//console.log('stats.pool.networkSols :'+pool.networkSols.map(x => x))
	...
	var diff =	getScaledNetworkDiff(poolName in stats.pools ? stats.pools[poolName].poolStats.networkDiff : 0);
        var pair = 	getReadableHashRatePair(poolName in stats.pools ? stats.pools[poolName].hashrate:0);
	...
	$("#blockReward").html(poolName + '<BR>Block Reward<BR>' + reward  );
	...
	$("#poolHashRate").text((!isNaN(hash) ? hash : 0) + ' ' + (pair[1] ? pair[1] : 'Sols/s'));
        ...
        $("#lastBlockFoundTime").html(poolName in stats.pools ? readableDate(stats.pools[poolName]?.blocks?.lastBlockTime*1000) : 0)  //[0]?.split(':')[4] * 1000) || 0 : 0);
		...
		var payout=(totalPaid*parseFloat(stats.pools[poolName].poolStats.coinMarketCap)).toFixed(2);
		payout='Payout Value<BR>$'+payout+' USD';
	$("#poolPaidOut").html(payout);
        $("#blocksFound").html(poolName in stats.pools ? 'Blocks Found</br>'+ stats.pools[poolName]?.blocks?.blocksFound : 0);
...
//	$("#bigDiff").html(poolName in stats.pools ?   'BigDiff Blocks</br>'+ parseInt(stats.pools[poolName]?.bigDiff) : 0);
        ...
        addChartData(poolHashrateChart, poolHashrateChart.data.datasets[1], {t: time,y: getReadableHashRate(avg)}, true);
        addChartData(poolHashrateChart, poolHashrateChart.data.datasets[2], {t: time,y: diff}, true);
//	addChartData(networkDiffChart, networkDiffChart.data.datasets[0], {t: time, y: getScaledNetworkDiff(stats.pools[poolName].poolStats.networkDiff)}, true);
//	addChartData(networkDiffChart, networkDiffChart.data.datasets[1], {t: time, y: getScaledNetworkDiff(stats.pools[poolName].poolStats.networkSols)}, true);
var text = 'block';
var now=Date.now();
var then=now-60*60;
