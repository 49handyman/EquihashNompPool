...
// var marketStats = require('./libs/marketStats.js');
...
var startmarketStats = function(){
 console.log('\u001b[35m init.js called startmarketStats ');
     var enabledForAny = false;
     for (var pool in poolConfigs){
         var p = poolConfigs[pool];
         var enabled = p.enabled && p.marketStats;
console.log('\u001b[35m startmarketStats is Enabled ', pool, enabled, p.enabled, p.marketStats)
         if (enabled){
     ...
     if (!enabledForAny)
console.log('\u001b[35m startmarketStats  if (!enabledForAny) Enabled ', enabledForAny)
         return;
...
console.log('\u001b[35m startmarketStats cluser.fork ', poolConfigs)
     worker.on('exit', function(code, signal){
